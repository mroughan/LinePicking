...
[g1] = distance_dist(t, 2, [1, 1]);
[g2] = distance_dist(t, 4, 2);
figure(1)
hold off
...
[g1] = distance_dist(t, 1, 1);
[g2] = distance_dist(t, 2, [2, 1]);
figure(2)
hold off
p1 = plot(t, g1, 'linewidth', 2);
hold on
...
[g1] = distance_dist(t, 0, 1);
[g2] = distance_dist(t, 3, [1, 1]);
figure(3)
hold off
p1 = plot(t, g1, 'linewidth', 2);
hold on
...
[g1] = distance_dist(t, 4, 1);
[g2] = distance_dist(t, 3, [0.00001, 1]);
figure(4)
hold off
p1 = plot(t, g1, 'linewidth', 2);
hold on
...
xlabel('t');
ylabel('g_n(t)');
print('-depsc', 'Plots/distance_dist_test_balls.eps');
...
[g_square] = distance_dist(t, 0, 1/sqrt(2));
[g_disk] = distance_dist(t, 1, 0.5);
[g_sphere] = distance_dist(t, 2, [3,0.5]);
[g_rect] = distance_dist(t, 3, [0.5,1]/sqrt(1.25));
[g_line] = distance_dist(t, 4, 1);
[g_cube] = distance_dist(t, 5, 1/sqrt(3));
figure(6)
hold off
plot(0,0)
hold on
...
xlabel('t');
ylabel('g(t)');
print('-depsc', 'Plots/distance_dist_test_fix_max_len.eps');
...
dt = 0.001;
t = -0.1:dt:3.1;
[g_square] = distance_dist(t, 0, 1);
[g_disk] = distance_dist(t, 1, 1/sqrt(pi));
[g_sphere] = distance_dist(t, 2, [3,(3/(4*pi))^(1/3)]);
[g_rect] = distance_dist(t, 3, [0.5,1]*2);
[g_line] = distance_dist(t, 4, 1);
[g_cube] = distance_dist(t, 5, 1);
figure(7)
hold off
plot(0,0)
hold on
...
xlabel('t');
ylabel('g(t)');
print('-depsc', 'Plots/distance_dist_test_fix_area.eps');
...
hold on
for n=1:8
  [g_rect] = distance_dist(t, 3, [n,1]/sqrt(1+n^2));
...
xlabel('t');
ylabel('g_{a/b}(t)');
print('-depsc', 'Plots/distance_dist_test_rect.eps');
...
for i=1:length(Ss)
  s = Ss(i);
  g_square = @(t) exp(-s*t) .* distance_dist(t, 0, 1);
  g_disk = @(t) exp(-s*t) .* distance_dist(t, 1, R);
  g_sphere = @(t) exp(-s*t) .* distance_dist(t, 2, [3,(3/(4*pi))^(1/3)]);
  g_rect = @(t) exp(-s*t) .* distance_dist(t, 3, [0.5,1]*2);
  g_line = @(t) exp(-s*t) .* distance_dist(t, 4, 1);
  g_cube = @(t) exp(-s*t) .* distance_dist(t, 5, 1);
...
xlabel('s');
ylabel('G(s)');
print('-depsc', 'Plots/distance_dist_test_laplace_tranforms.eps');

for i=1:length(Ss)
  s = Ss(i);
  g_square = @(t) t .* exp(-s*t) .* distance_dist(t, 0, 1);
  g_disk = @(t) t .* exp(-s*t) .* distance_dist(t, 1, R);
  g_sphere = @(t) t .* exp(-s*t) .* distance_dist(t, 2, [3,(3/(4*pi))^(1/3)]);
  g_rect = @(t) t .* exp(-s*t) .* distance_dist(t, 3, [0.5,1]*2);
  g_line = @(t) t .* exp(-s*t) .* distance_dist(t, 4, 1);
  g_cube = @(t) t .* exp(-s*t) .* distance_dist(t, 5, 1);
  [q_square, errbnd] = quadgk(g_square, t_min, t_max);
  [q_disk, errbnd] = quadgk(g_disk, t_min, t_max);
  [q_sphere, errbnd] = quadgk(g_sphere, t_min, t_max);
  [q_rect, errbnd] = quadgk(g_rect, t_min, t_max);
...
xlabel('s');
ylabel('-G''(s)/G(s)');
print('-depsc', 'Plots/distance_dist_test_laplace_ratio.eps');
...
xlabel('s');
ylabel('-G''(s)/G(s)');
print('-depsc', 'Plots/distance_dist_test_quad.eps');
...
print('-depsc', 'Plots/distance_dist_approx_error.eps');
...
print('-depsc', 'Plots/distance_dist_approx_rel_error.eps');
