...
while (<FILE>) {
  if ($mex > 0) {
	...
	$descriptions{$gateway} = $2;
      ...
      print "case=$case_i, \t gateway=$gateway, \t description=$descriptions{$gateway}\n";
    } elsif (/default:/) {
      $case_i = 0;
    }
    ...
    if (/$RE{comment}{C}{-keep}/) {
      if ($1 =~ /(input|output) argument (\d+):([^,]+),\s*([^*]*)/) {
	$inp_out = $1;
	$number = $2;
	$name = $3;
	$description = $4;
	print "               \$inp_out=$inp_out, \$number=$number, \$name=$name, \$description=$description\n";
...
$name_list = join(', ', (sort {$a cmp $b} (values %gateways)));
